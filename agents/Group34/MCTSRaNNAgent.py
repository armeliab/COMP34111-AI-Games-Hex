...
class MCTSRaNNAgent(AgentBase):
    """
    Python wrapper for MCTS+RAVE+Neural Network C++ agent.
    Uses hybrid approach: rollout early game, neural network value late game.
    """
    ...
    def __init__(self, colour: Colour):
        ...
        if sys.platform == "win32":
            self._binary_path = bin_dir / "mcts_rave_nn_agent.exe"
        else:
            self._binary_path = bin_dir / "mcts_rave_nn_agent"
    ...
    def _launch_process(self, board_size: int):
        ...
        self.agent_process = Popen(
            [
                ...
                "200.0",  # RAVE_K
                sys.executable,  # python
                str(bin_dir / "nn_server.py"),
                str(bin_dir / "weights.pt"),
                "45",   # VALUE_SWITCH_EMPTY
                "0.8",  # C_PUCT
            ],
